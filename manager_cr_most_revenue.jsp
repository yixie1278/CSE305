    ...
    <span style="font-size: 14pt; font-family: Arial"><strong>Hello, Manager. Your Email is
        ...
        <%
          ...
          try{
            ...
            java.sql.ResultSet rs = stmt1.executeQuery("SELECT * FROM(SELECT p.SSN, p.FirstName, p.LastName ,sum(s.Price) as TotalRevenue FROM `sales` s, `auction` a, `Person` p WHERE s.AuctionID = a.AuctionID and a.Monitor = p.SSN GROUP BY p.SSN ORDER BY TotalRevenue) t1 WHERE TotalRevenue >= ( SELECT sum(s.Price) as TotalRevenue FROM `sales` s, `auction` a, `Person` p WHERE s.AuctionID = a.AuctionID and a.Monitor = p.SSN GROUP BY p.SSN order by TotalRevenue DESC LIMIT 1)");
