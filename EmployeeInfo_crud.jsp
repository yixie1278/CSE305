...
<head>
    <title>Manager Level Transaction - <%=request.getParameter("type")%></title>
    <script language="javascript" type="text/javascript">
        // <!CDATA[
        function Button1_onclick() {
          if(document.myForm.startDate.value == "")
          alert("Your startDate must not be empty!!!")
          else if(document.myForm.hourlyRate.value == "")
          alert("Your hourly rate must not be empty!!!")
          else if(document.myForm.level.value == "")
          alert("Your level  must not be empty!!!")
        ...
        //    alert("UserName Can Not Be Null!")
            ...
            alert("Hourly Rate can't be null")
            else if(document.myForm.level.value == "")
            alert("Level can't be null")
            else{document.myForm.submit()}        
        }
        </script>
</head>
<body style="text-align: center" bgcolor="#ffffff">
    <span style="font-size: 14pt; font-family: Arial"><strong>Hello, Manager. Your Email is
        <%=session.getValue("login")%>. Your ID is <%=session.getValue("employeeID")%> Here is the Employee Information You want to update.<br />
        <br />
    </span>
    ...
    <%
          ...
          try{
            ...
            java.sql.ResultSet rs = stmt1.executeQuery("select * from employee , person where EmployeeID ="+request.getParameter("id")+" and EmployeeID = SSN");
        
            while(rs.next()){
              if(request.getParameter("type").equals("edit")){        
    %>
                <h1><%=request.getParameter("type")%> - SSN: <%=request.getParameter("id")%></h1>
                <form name="myForm" action="employee_crud_transaction.jsp" method="post">
                                <input type="hidden" name="target" value="update">
                           
                               

                                <span style="font-size: 10pt"><strong> <br />StartDate:<br /></strong></span>
                                <input name="startDate" style="width: 145px" type="text" value ="<%=rs.getString("StartDate")%>"/><br />

                                
                                <span style="font-size: 10pt"><strong>Hourly Rate:<br />
                                <input name="hourlyRate" style="width: 145px" type="text" value="<%=rs.getString("HourlyRate")%>" /><br />
                                
                                <span style="font-size: 10pt"><strong>Level:<br />
                                <input name="level" style="width: 145px" type="text" value="<%=rs.getString("Level")%>" /><br />
                                <br><br>
            ...
            %>
              <h1><%=request.getParameter("type")%></h1>
                <form name="myForm" action="employee_crud_transaction.jsp" method="post">
                                ...
                                <span style="font-size: 10pt"><strong> <br />StartDate:<br /></strong></span>
                                <input name="startDate" style="width: 145px" type="text" /><br />

                                
                                <span style="font-size: 10pt"><strong>Hourly Rate:<br />
                                <input name="hourlyRate" style="width: 145px" type="text"  /><br />
                                
                                <span style="font-size: 10pt"><strong>Level:<br />
                                <input name="level" style="width: 145px" type="text"  /><br />
                                <br><br>
