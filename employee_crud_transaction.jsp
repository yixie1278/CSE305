...
*/
			...
			try 
			{
	            ...
	            if (request.getParameter("target").trim().equals("update")){
					...
					stmt1.executeUpdate("UPDATE `employee` SET `Level`="+level+", `HourlyRate` = "+hourlyRate+" , `StartDate` = '"+startDate+"' WHERE  `EmployeeID`="+ssn+";");
					...
					String ssn = request.getParameter("ssn");
					stmt1.executeUpdate("DELETE FROM `employee` WHERE  `EmployeeID`="+ssn+";");
					...
					*/
					...
					stmt1.executeUpdate("INSERT INTO `employee` ( `EmployeeID`,`Level`,	`StartDate`,`HourlyRate`) VALUES ("+ssn+", "+level+",'"+startDate+"', '"+hourlyRate+"');");
					/*stmt1.executeUpdate("INSERT INTO `person` (`SSN`, `LastName`, `FirstName`, `Address`, `ZipCode`, `telephone`, `email`, `password`) VALUES (					'"+ssn+"' ,					'"+lastName+"', 					'"+firstName+"', 					'"+address+"', 					"+zipCode+", 					"+phone+", 					'"+email+"',					'"+Password1+"'		);");
...
Successful executed!
<a href="Manager_crud.jsp"><font color="Blue">Manager Page</font></a>